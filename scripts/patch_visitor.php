<?php
$path = __DIR__ . '/../templates/auth/visitor.html.twig';
$lines = file($path);
$start = null; $end = null;
foreach ($lines as $i => $l) {
    if (strpos($l, '{% if random_courses') !== false) {
        $start = $i;
    }
    if ($start !== null && strpos($l, '{% endif %}') !== false && $end === null && $i > $start) {
        $end = $i;
        break;
    }
}
...
$replacement_lines = array_map(function($l){ return $l . "\n"; }, explode("\n", $new_block));

$before = array_slice($lines, 0, $start);
$after = array_slice($lines, $end+1);
$new_lines = array_merge($before, $replacement_lines, $after);
file_put_contents($path, implode('', $new_lines));
echo "replaced block lines $start-$end\n";
